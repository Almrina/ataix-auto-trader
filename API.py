...
def get_highest_bid(symbol):
    ...
    for pair in symbols:
        print(f"Checking pair: {pair}")  # Отладка: смотрим структуру данных
...
def create_order(symbol, price, amount):
    ...
    print(f"Order response: {response}")  # Вывод ответа API для проверки
...
def main():
    ...
    num_orders = 2
    amount_per_order = usdt_balance / (num_orders * highest_bid)  # Равномерное распределение средств

    orders = []

    for i, price in enumerate(price_levels):  # price теперь берется из списка
    ...
    time.sleep(1)  # Задержка перед следующим ордером
